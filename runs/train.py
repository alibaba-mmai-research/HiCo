...
def write_current_epoch(current_epoch, total_epoch):
    path = "./current_epoch/"
    if os.path.exists(path) is False:
        os.makedirs(path)
    exist_file = os.listdir(path)
    os.system(f"touch {path}{current_epoch}-{total_epoch}")
    for f in exist_file:
        if f != f"{current_epoch}-{total_epoch}":
...
def train_epoch(
    ...
    write_current_epoch(cur_epoch, cfg.OPTIMIZER.MAX_EPOCH)
    for cur_iter, (inputs, labels, indexes, meta) in enumerate(train_loader):
        if (cur_iter+1) % 20 == 0:
            write_current_epoch(cur_epoch + cfg.TRAIN.NUM_FOLDS * float(cur_iter) / data_size, cfg.OPTIMIZER.MAX_EPOCH)
